...
flags.DEFINE_enum(
    "mode", None, ["jax_cpu", "jax_gpu", "jax_tpu"], "Distributed strategy approach."
)
...
def process_batch(
    *,
    ...
    if save_dir:
        ...
        image_utils.save_depth(save_dir / f"depth_expected_{item_id}.png", depth_exp)
        image_utils.save_image(
            save_dir / f"depth_median_viz_{item_id}.png",
            image_utils.image_to_uint8(depth_med_viz),
        )
        image_utils.save_depth(save_dir / f"depth_median_{item_id}.png", depth_med)
    ...
    summary_writer.image(f"disparity-expected/{tag}/{item_id}", disp_exp_viz, step)
...
def main(argv):
    ...
    logging.info(
        "Starting host %d. There are %d hosts : %s",
        jax.process_index(),
        jax.process_count(),
    ...
    logging.info("Found %d total devices: %s.", jax.device_count(), str(jax.devices()))
