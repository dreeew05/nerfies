...
class TranslationField(nn.Module):
    ...
    def encode_metadata(
        self, metadata: jnp.ndarray, time_alpha: Optional[float] = None
    ):
        if self.metadata_encoder_type == "time":
            ...
            metadata_embed = (1.0 - time_alpha) * glo_embed + time_alpha * time_embed
    ...
    def warp(
        self, points: jnp.ndarray, metadata_embed: jnp.ndarray, extra: Dict[str, Any]
    ):
    ...
    def __call__(
        self,
        points: jnp.ndarray,  # During init, points are JAX arrays
        metadata: jnp.ndarray,  # During init, metadata entries are JAX arrays
        extra: Dict[str, Any],
        return_jacobian: bool = False,
        metadata_encoded: bool = False,
    ):

        print(f"--- Entering {self.__class__.__name__}.__call__ ---")
        print(
            f"points type: {type(points)}, shape: {points.shape if hasattr(points, 'shape') else 'N/A'}"
        )
        print(f"metadata type: {type(metadata)}")  # Should be dict
        # Check type of specific metadata entries if accessed
        # print(f"metadata.get('warp') type: {type(metadata.get('warp'))}") # Should be JAX array during init

        if metadata_encoded:
            metadata_embed = metadata
        else:
            # Call to encode_metadata happens here
            print("Calling encode_metadata...")
            metadata_embed = self.encode_metadata(metadata, extra.get("time_alpha"))
            print("encode_metadata called.")

        print(
            f"metadata_embed type: {type(metadata_embed)}, shape: {metadata_embed.shape if hasattr(metadata_embed, 'shape') else 'N/A'}"
        )

        out = {
            "warped_points": self.warp(points, metadata_embed, extra)
        }  # Call to warp happens here

        # ... rest of __call__
        print(f"--- Exiting {self.__class__.__name__}.__call__ ---")
        return out
